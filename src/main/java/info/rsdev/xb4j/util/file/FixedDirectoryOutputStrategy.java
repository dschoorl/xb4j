...
/**
 * This class is responsible for providing temporary {@link File} instances pointing to empty but existing files to which the
 * application can write. The files created are marked {@link File#deleteOnExit()}.
 *
 * @author Dave Schoorl
 */
public class FixedDirectoryOutputStrategy implements IFileOutputStrategy {
    ...
    /**
     * Create a new {@link FixedDirectoryOutputStrategy} that will use java's temp directory as target. This constructor is added to
     * create a static {@link FixedDirectoryOutputStrategy} {@link #INSTANCE}, which surpresses exceptions when it can not write to
     * that directory.
     */
