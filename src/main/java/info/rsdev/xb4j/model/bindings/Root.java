...
/**
 * <p>
 * This Binding is at the root of a binding hierarchy. It has a reference to the {@link BindingModel}, so it can lookup
 * {@link ComplexType complextype definitions}.</p>
 *
...
public class Root extends Element implements IModelAware, ISemaphore {
    ...
    /**
     * Flag that indicates whether this {@link Root} element can be changed or not. A root element is made immutable, the first time
     * it is used to marshall/unmarshall, so that it can be used in a threadsafe manner. When a {@link #copy(QName)} is made, the
     * copy is mutable again.
     */
    ...
    /**
     * Copy this {@link Root}, so it can be registered for another element (E.g. another namespace), but with the same structure).
     * The BindingModel of this RootBinding (if any) will not be copied, so you can register this copy also with another
     * BindingModel.
     *
     * @param newElement
