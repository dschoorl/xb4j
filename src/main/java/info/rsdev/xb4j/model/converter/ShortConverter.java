...
public class ShortConverter implements IValueConverter {
    ...
    /**
     * Create a new {@link ShortConverter} that performs validation on the Short. If you don't need validation, please use the
    ...
    /**
     * Create a new {@link ShortConverter} with a specific {@link IValidator} and a minimum required length. When converting xml to
     * Object, a {@link ValidationException} is thrown when the length is shorter than minLength. When converting an {@link Short}
     * to xml, the number will be left padded with zeros to reach the minLength size.
     *
     * @param validator
     * @param minLength
     */
    ...
    public static final class RangeValidator implements IValidator {
        ...
        public <T> T isValid(T instance) throws ValidationException {
            ...
            if ((value < lowerBound) || (value > upperBound)) {
                throw new ValidationException(String.format("Short must be between %d and %d: %d", lowerBound, upperBound, value));
            }
            return instance;
        }
